...
const navigation = [['Home', '#home'], ['Feature', '#features'], ['How It Work', '#how-it-works'], ['About Us', '#why-aurtx'], ['Contact', '#contact']]
const features = [
  ...
  return <div className="homepage" id="home">
    ...
    <div className="hero-wrap"><section className="hero" aria-labelledby="hero-title">
      <header className="site-header"><a href="#home" aria-label="Aurtx Digital home"><img src={icon('logo')} width="86" height="64" alt="Aurtx Digital" /></a>
        <button ref={menuButton} className="menu-toggle" type="button" aria-label={menuOpen ? 'Close navigation' : 'Open navigation'} aria-expanded={menuOpen} aria-controls="main-navigation" onClick={() => setMenuOpen(!menuOpen)}><span /><span /><span /></button>
        <nav id="main-navigation" aria-label="Main navigation" className={menuOpen ? 'navigation is-open' : 'navigation'}>{navigation.map(([label, href]) => <a key={label} href={href} onClick={() => setMenuOpen(false)}>{label}</a>)}</nav>
    ...
    </section><dl className="statistics">{[['1', 'Workspace for your team'], ['24/7', 'Client conversations'], ['AI', 'Reply assistance'], ['100%', 'Team visibility']].map(([number, label]) => <div key={number}><dt>{number}</dt><dd>{label}</dd></div>)}</dl></div>
    <main id="main-content">
      <section className="video-section" id="how-it-works" aria-labelledby="video-title"><div className="section-heading"><span className="eyebrow pill">AURTX DIGITAL</span><h2 id="video-title">See How Aurtx Makes Client Work Simpler</h2><p>Bring your WhatsApp conversations, team, tasks, and AI assistance together in one workspace built to keep your business moving</p></div><div className="video-previews">{['WhatsApp phone workspace', 'Team member using the Aurtx dashboard', 'Aurtx dashboard on a desktop monitor'].map((alt, index) => <img key={alt} src={picture(`video-placeholder${index + 1}.png`)} alt={`${alt} — static video preview`} width={index === 0 ? 386 : 387} height="340" loading="lazy" />)}</div></section>
      ...
      <section className="product-section" id="products" aria-labelledby="product-title"><div className="product-panel"><div className="section-heading"><h2 id="product-title">THE PRODUCTS</h2><p>From managing client conversations to coordinating your team and getting AI assistance, Aurtx gives your business the tools to work smarter from one place.</p></div><div className="product-carousel" role="region" aria-label="Aurtx products" tabIndex={0} onKeyDown={event => { if (event.key === 'ArrowRight') { event.preventDefault(); slider.current?.slideNext() } else if (event.key === 'ArrowLeft') { event.preventDefault(); slider.current?.slidePrev() } }}>
        <Swiper modules={[A11y, Navigation]} onSwiper={instance => { slider.current = instance }} slidesPerView={1} spaceBetween={24} breakpoints={{ 640: { slidesPerView: 2, spaceBetween: 24 }, 1100: { slidesPerView: 3, spaceBetween: 48 } }} speed={reducedMotion ? 0 : 350} navigation={{ prevEl: '.product-previous', nextEl: '.product-next' }} a11y={{ containerMessage: 'Aurtx products. Use left and right arrow keys to browse.' }}>{products.map(([name, description, width, height], index) => <SwiperSlide key={name}><article className={`product-card product-${index + 1}`}><img className="product-image" src={picture(`products-img${index + 1}.png`)} width={width} height={height} alt={`${name} workspace preview`} loading="lazy" /><div className="product-label"><span className="product-mark"><img src={icon('logo-white')} width="93" height="64" alt="" /></span><div><h3>{name}</h3><p>{description}</p></div></div></article></SwiperSlide>)}</Swiper>
        <div className="product-controls"><button type="button" className="product-previous" aria-label="Previous product">←</button><button type="button" className="product-next" aria-label="Next product">→</button></div>
      </div></div></section>
      <section className="why-section" id="why-aurtx" aria-labelledby="why-title"><div className="section-heading"><span className="eyebrow">WHY CHOOSE AURTX</span><h2 id="why-title">Everything Your Team Needs to Work Smarter</h2></div><div className="reason-grid">{reasons.map(([title, description], index) => <article key={title} tabIndex={index === 5 ? 0 : undefined} className={`reason-card${index === 5 ? ' hover-card' : ''}`}><span className="number-badge">{index + 1}</span><h3>{title}</h3><p>{description}</p></article>)}</div></section>
