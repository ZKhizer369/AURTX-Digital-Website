...
export function TeamRolesSection() {
  return <section className="roles-section px-6 pt-12 pb-16 [@media(max-width:639px)]:px-5 [@media(max-width:639px)]:pt-8 [@media(max-width:639px)]:pb-12 [&_h2]:font-bold!" aria-labelledby="roles-title">
    ...
    <div className="roles-grid mx-auto mt-9 grid max-w-305 gap-6 grid-cols-3 [@media(max-width:1099px)]:grid-cols-2 [@media(max-width:639px)]:mt-7 [@media(max-width:639px)]:grid-cols-1">{roles.map(([title, description], index) => <article className="role-card rounded-[20px] border border-[#dde9e5] bg-[#f2f2f2] p-6 [&>h3]:text-lg [&>h3]:leading-[1.4] [&>h3]:font-semibold [&>p]:mt-1.25 [&>p]:text-[15px] [&>p]:leading-[1.7] [&>p]:text-[#555] min-h-48.75" key={title}>
      <span className="role-icon mb-4 flex h-11.5 w-11.5 items-center justify-center rounded-xl bg-[#063d39] text-lg text-white [&>img]:h-auto [&>img]:w-auto [&>img]:max-h-8.25 [&>img]:max-w-9.25">
